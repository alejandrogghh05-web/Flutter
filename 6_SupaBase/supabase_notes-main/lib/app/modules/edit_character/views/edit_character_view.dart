...
class EditFavoriteCharacterView extends GetView<EditFavoriteCharacterController> {
  FavoriteCharacters character = Get.arguments;
  HomeController homeC = Get.find();

  EditFavoriteCharacterView({super.key});
  
  @override
  Widget build(BuildContext context) {
    // Pre-cargar los datos del personaje
    controller.nameC.text = character.name ?? '';
    controller.originC.text = character.origin ?? '';
    controller.descriptionC.text = character.description ?? '';
    controller.notesC.text = character.notes ?? '';
    controller.selectedCharacterType.value = character.characterType ?? 'Protagonist';
    
    return Scaffold(
      appBar: AppBar(
        title: const Text('Edit Favorite Character'),
        centerTitle: true,
      ),
      body: ListView(
          ...
          Obx(() => ElevatedButton(
            onPressed: () async {
              if (controller.isLoading.isFalse) {
                bool res = await controller.editFavoriteCharacter(character.id!);
                if (res == true) {
                  await homeC.getAllCharacters();
                  Get.back();
                  Get.snackbar(
                    'Success',
